...
const SignUp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      const message = err?.data?.message || err?.data?.error || err?.error || err?.status || 'Registration failed. Please try again.';
      setError(message);
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-slate-900 dark:to-slate-800 py-12 px-4">
      ...
      <div
        ...
        style={{
          backgroundImage:
            "url('data:image/svg+xml,%3Csvg%20width=%2760%27%20height=%2760%27%20viewBox=%270%200%2060%2060%27%20xmlns=%27http://www.w3.org/2000/svg%27%3E%3Cg%20fill=%27none%27%20fill-rule=%27evenodd%27%3E%3Cg%20fill=%27%239C92AC%27%20fill-opacity=%270.05%27%3E%3Ccircle%20cx=%2730%27%20cy=%2730%27%20r=%272%27/%3E%3C/g%3E%3C/g%3E%3C/svg%3E')",
        }}
      ></div>
          ...
          {error && (
            <div className="mb-6 p-4 rounded-2xl bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 flex items-center gap-3 text-red-600 dark:text-red-400">
              ...
              {error === 'Email already registered' ? (
                <p className="text-sm">
                  Email already registered. Please <Link to="/login" className="underline hover:text-red-700">login</Link> instead.
                </p>
              ) : (
